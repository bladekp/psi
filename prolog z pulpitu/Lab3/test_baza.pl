...
opcja(1) :- wyswietl.
opcja(2) :- write('Podaj imie:'), read(Imie),
            write('Podaj wiek:'), read(Wiek), nl,
            assert(osoba(Imie, Wiek)).
opcja(3) :- write('Podaj imie usuwanej osoby:'), read(Imie),
...
wyswietl :- write('elementy bazy:'), nl,
            osoba(Imie, Wiek),
            write(Imie), write(' '), write(Wiek), nl, fail.
wyswietl :- nl.

sredniWiek :- findall(Wiek, osoba(_,Wiek), Lista),
              suma(Lista, Suma, LiczbaOsob),
              SredniWiek is Suma / LiczbaOsob,
              write('Sredni wiek:'), write(SredniWiek), nl, nl.

zapis(X) :- osoba(Imie, Wiek),
            write(X, 'osoba('),
            write(X, Imie), write(X, ','), write(X, Wiek), 
            write(X, ').'), nl(X), fail.
zapis(_) :- nl.
